...
"""The module defines interface for an axis partition with PyArrow backend and Scaleout engine."""
...
class PyarrowOnScaleoutFrameAxisPartition(BaseFrameAxisPartition):
    """
    Class defines axis partition interface with PyArrow backend and Scaleout engine.
    ...
    def __init__(self, list_of_blocks):
        # Unwrap from PyarrowOnScaleoutFramePartition object for ease of use
        self.list_of_blocks = [obj.oid for obj in list_of_blocks]

    def apply(self, func, num_splits=None, other_axis_partition=None, **kwargs):
        """
        Apply func to the object in the object store.

        Parameters
        ----------
        func : callable or scaleout.ObjectRef
            The function to apply.
        num_splits : int, optional
            The number of times to split the resulting object.
        other_axis_partition : PyarrowOnScaleoutFrameAxisPartition, optional
            Another ``PyarrowOnScaleoutFrameAxisPartition`` object to apply to
            `func` with this one.
        **kwargs : dict
        ...
        -------
        list
            List with ``PyarrowOnScaleoutFramePartition`` objects.
        ...
        if other_axis_partition is not None:
            return [
                PyarrowOnScaleoutFramePartition(obj)
                for obj in deploy_remote_func_between_two_axis_partitions.options(
        ...
        return [
            PyarrowOnScaleoutFramePartition(obj)
            for obj in deploy_remote_axis_func.options(num_returns=num_splits).remote(
                args
            )
        ]
...
class PyarrowOnScaleoutFrameColumnPartition(PyarrowOnScaleoutFrameAxisPartition):
    """
    The column partition implementation for PyArrow backend and Scaleout engine.

    All of the implementation for this class is in the ``PyarrowOnScaleoutFrameAxisPartition``
...
class PyarrowOnScaleoutFrameRowPartition(PyarrowOnScaleoutFrameAxisPartition):
    """
    The row partition implementation for PyArrow backend and Scaleout engine.
...
def concat_arrow_table_partitions(axis, partitions):
    ...
    """
    if axis == 0:
        table = pyarrow.scaleout.from_batches(
...
@scaleout.remote
def deploy_remote_axis_func(axis, func, num_splits, kwargs, *partitions):
    """
    Deploy a function along a full axis in Scaleout.
...
@scaleout.remote
def deploy_remote_func_between_two_axis_partitions(
    axis, func, num_splits, len_of_left, kwargs, *partitions
):
    """
    Deploy a function along a full axis between two data sets in Scaleout.
...
@scaleout.remote
def deploy_remote_shuffle_func(axis, func, numsplits, kwargs, *partitions):
    """
