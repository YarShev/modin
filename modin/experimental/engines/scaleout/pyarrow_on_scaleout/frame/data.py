...
"""
Module contains class ``PyarrowOnScaleoutFrame``.

``PyarrowOnScaleoutFrame`` is a dataframe class with PyArrow backend and Scaleout engine.
"""
...
class PyarrowOnScaleoutFrame(PandasFrame):
    """
    Class for dataframes with PyArrow backend and Scaleout engine.

    ``PyarrowOnScaleoutFrame`` implements interfaces specific for PyArrow and Scaleout,
    ...
    def to_pandas(self):
        ...
        pandas.DataFrame
        """
        df = super(PyarrowOnScaleoutFrame, self).to_pandas()
        df.index = self.index
