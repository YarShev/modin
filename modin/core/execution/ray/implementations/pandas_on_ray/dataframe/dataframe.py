...
class PandasOnRayDataframe(PandasDataframe):
    ...
    def __init__(
        ...
        self._index_cache = (
            ...
            and not any(isinstance(idx, ObjectIDType) for idx in index)
            else index
        )
        self._columns_cache = (
            ensure_index(columns)
            if columns is not None
            and not any(isinstance(col, ObjectIDType) for col in columns)
            else columns
        )
        super().__init__(
            partitions,
    ...
    def _validate_axes_lengths(self):
        ...
        if (
            ...
            and not any(
                isinstance(cache, ObjectIDType) for cache in self._row_lengths_cache
            )
        ...
        if (
            ...
            and not any(
                isinstance(cache, ObjectIDType) for cache in self._column_widths_cache
            )
    ...
    def _row_lengths(self):
        ...
        """
        if self._row_lengths_cache is not None and any(
            isinstance(cache, ObjectIDType) for cache in self._row_lengths_cache
        ):
            if len(self._partitions) > 0:
                self._row_lengths_cache = [
                    len(ray.get(length)) for length in self._row_lengths_cache
                ]
            else:
                self._row_lengths_cache = []
    ...
    def _column_widths(self):
        ...
        """
        if self._column_widths_cache is not None and any(
            isinstance(cache, ObjectIDType) for cache in self._column_widths_cache
        ):
            if len(self._partitions) > 0:
                self._column_widths_cache = [
                    len(ray.get(width)) for width in self._column_widths_cache
                ]
            else:
                self._column_widths_cache = []
    ...
    def _make_init_labels_args(self, partitions, index, columns) -> dict:
        ...
        kw["index"], kw["row_lengths"] = (
            self._compute_axis_labels_and_lengths_async(0, partitions)
            if index is None
            else (index, None)
        )
        kw["columns"], kw["column_widths"] = (
            self._compute_axis_labels_and_lengths_async(1, partitions)
            if columns is None
            else (columns, None)
        )
        return kw
    ...
    def _get_index(self):
        ...
        """
        if self._index_cache is not None and any(
            isinstance(cache, ObjectIDType) for cache in self._index_cache
        ):
            lengths = [ray.get(length) for length in self._index_cache]
            self._index_cache = lengths[0].append(lengths[1:])
    ...
    def _get_columns(self):
        ...
        if isinstance(self._columns_cache, ObjectIDType):
            widths = [ray.get(width) for width in self._columns_cache]
            self._columns_cache = widths[0].append(widths[1:])
    ...
    def _set_index(self, new_index):
        ...
        if self._index_cache is None:
            ...
            if any(isinstance(cache, ObjectIDType) for cache in self._index_cache):
                lengths = [ray.get(length) for length in self._index_cache]
                self._index_cache = lengths[0].append(lengths[1:])
    ...
    def _set_columns(self, new_columns):
        ...
        if self._columns_cache is None:
            ...
            if any(isinstance(cache, ObjectIDType) for cache in self._columns_cache):
                widths = [ray.get(width) for width in self._columns_cache]
                self._columns_cache = widths[0].append(widths[1:])
