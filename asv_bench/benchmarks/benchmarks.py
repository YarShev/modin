...
CONCAT_DATA_SIZE = [(10_128, 1000, 10_128, 1000)]
...
class TimeDot:
    ...
    def setup(self, impl, data_type, data_size):
        ...
        self.df2 = generate_dataframe(
            impl, data_type, data_size[1], data_size[0], RAND_LOW, RAND_HIGH
        )

    def time_dot(self, impl, data_type, data_size):
        self.df1.dot(self.df2)


class TimeCorr:
    param_names = ["impl", "data_type", "data_size", "min_periods"]
    params = [
        ["modin"],
        ["float"],
        CORR_DATA_SIZE,
    ...
    def setup(self, impl, data_type, data_size, min_periods):
        self.df = generate_dataframe(
            impl, data_type, data_size[0], data_size[1], RAND_LOW, RAND_HIGH
        )

    def time_corr(self, impl, data_type, data_size, min_periods):
        self.df.corr(min_periods=min_periods)
...
class TimeConcat:
    ...
    def setup(self, data_type, data_size, how, axis):
        self.df1 = generate_dataframe(
            "modin", data_type, data_size[0], data_size[1], RAND_LOW, RAND_HIGH
        )
        self.df2 = generate_dataframe(
            "modin", data_type, data_size[2], data_size[3], RAND_LOW, RAND_HIGH
        )
